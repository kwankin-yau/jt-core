...
import info.gratour.common.utils.{BcdUtils, BitUtils}
...
object JT808Utils {
  ...
  def bcd6ToTimestamp(bcd: Array[Byte], offset: Int): Long = {
    ...
    val odt = OffsetDateTime.of(y, m, d, h, min, s, 0, Consts.ZONE_OFFSET_BEIJING)
  ...
  def timestampToBcd6(time: Long): Array[Byte] = {
    val odt = OffsetDateTime.ofInstant(Instant.ofEpochMilli(time), Consts.ZONE_OFFSET_BEIJING)
  ...
  def vtdrStrToLocalDateTime(s: String): LocalDateTime = {
    if (s == null) throw new ErrorWithCode(Errors.INVALID_VALUE, Errors.errorMessageFormat(Errors.INVALID_VALUE, "null"))

    var hex: Array[Byte] = null
    try hex = Hex.decodeHex(s)
    catch {
      case e: DecoderException =>
        throw new ErrorWithCode(Errors.INVALID_VALUE, Errors.errorMessageFormat(Errors.INVALID_VALUE, s))
    }

    if (hex.length != 6) throw new ErrorWithCode(Errors.INVALID_VALUE, Errors.errorMessageFormat(Errors.INVALID_VALUE, s))
