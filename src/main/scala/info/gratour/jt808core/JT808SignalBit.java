...
class JT808SignalNames {
    ...
    public static String getSignalName(int bitIndex) {
...
public enum JT808SignalBit {
    ...
    JT808SignalBit(int bitIndex) {
        this.index = bitIndex;
        this.signalName = JT808SignalNames.getSignalName(bitIndex);
    }
    ...
    public static String vehSignalText(int vehSignalBits) {
        StringBuilder str = new StringBuilder();
        boolean first = true;
        for (JT808SignalBit bit : values()) {
            if (BitUtils.test(vehSignalBits, bit.index)) {
                if (first)
                    first = false;
                else
                    str.append(";");
                str.append(bit.signalName);
            }
        }
