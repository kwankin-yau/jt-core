...
object MBEncoder808_8103_SetParam extends AbstractJT808MsgBodyEncoder[JT808Msg_8103_SetParam] {
  ...
  override def encodeBody(m: JT808Msg_8103_SetParam, out: ByteBuf): Unit = {
      ...
      if (dataType != null) {
        ...
        pid match {
          case 0x0064 =>
            ...
            for (i <- 1 to 5) {
              if (p.chanEnabled(i))
                bits = BitUtils.set(bits, 0 + i - 1)
              if (p.chanStored(i))
                bits = BitUtils.set(bits, 8 + i - 1)
            }
            if (p.getTimeUnit != 0)
              bits = BitUtils.set(bits, 16)
