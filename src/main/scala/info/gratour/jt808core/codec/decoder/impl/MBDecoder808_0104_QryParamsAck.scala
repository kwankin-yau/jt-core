...
import info.gratour.common.utils.{BitUtils, CommonUtils, StringUtils}
...
import info.gratour.jt808core.protocol.msg.types.termparams._
    ...
    (1 to count).foreach(_ => {
      ...
      id match {
        ...
        case 0x0064 =>
          ...
          for (i <- 1 to 5) {
            enabledFlags(i-1) = BitUtils.test(bits, 0 + i - 1)
            storedFlags(i-1) = BitUtils.test(bits, 8 + i - 1)
          }
          p.setChanEnabledFlags(enabledFlags)
          p.setChanStoredFlags(storedFlags)
          p.setTimeUnit(
            if (BitUtils.test(bits, 16))
              1
            else
              0
          )
        ...
        case x if (0x0110 to 0x01FF) contains x =>
          ...
          settings.setCalculated(BitUtils.test(l, 29))
          settings.setExtFrame(BitUtils.test(l, 30))
          settings.setCan2(BitUtils.test(l, 31))
        ...
        case _ =>
          ...
          map.put(idHex, StringUtils.hex(bytes, 0, len))
      }
