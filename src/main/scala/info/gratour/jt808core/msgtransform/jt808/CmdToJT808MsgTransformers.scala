...
import info.gratour.jt808core.msgtransform.{TerminalCmdToMsgTransformer, TerminalCmdToMsgTransformerRegistry}
import info.gratour.jt808core.protocol.JT808Msg
import info.gratour.jt808core.protocol.msg.types.cmdparams._
import info.gratour.jt808core.protocol.msg._
...
abstract class CmdToJT808MsgTransformers[T <: JT808Msg](implicit classTag: ClassTag[T]) extends TerminalCmdToMsgTransformer {
  ...
  override def toMsg(cmd: TermCmd): T = {
    ...
    val m = clzz.newInstance().asInstanceOf[T]
...
trait CmdParams {
  ...
  def getInt(name: String): Int = {
    ...
    o match {
      ...
      case s: String =>
        val i = StringUtils.tryParseInt(s)
        if (i == null)
