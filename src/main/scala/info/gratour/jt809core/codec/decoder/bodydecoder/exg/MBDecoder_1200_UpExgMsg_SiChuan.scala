...
object MBDecoder_1200_UpExgMsg_SiChuan extends VehRelatedMsgBodyDecoder {
  override def decodeSubBody(msg: JT809VehRelatedMsg, buf: ByteBuf): Unit = {
    msg match {
      ...
      case m: JT809Msg_1240_UpExgMsgSafetyTerminal_SiChuan =>
        m.setPlatformId(buf.readStrMaxLen(11))
        m.setProducer(buf.readStrMaxLen(50))
        m.setTerminalModel(buf.readStrMaxLen(30))
        m.setTerminalId(buf.readStrMaxLen(20))
        m.setInstallTime(buf.readLong())
        m.setInstallCompany(buf.readStrMaxLen(50))
        m.setContacts(buf.readStrMaxLen(20))
        m.setTel(buf.readStrMaxLen(20))
        m.setCompliance(buf.readByte() != 0)
    }
  }
  ...
  override def msgClasses: Seq[Class[_ <: JT809VehRelatedMsg]] = Seq(
    ...
    classOf[JT809Msg_1240_UpExgMsgSafetyTerminal_SiChuan]
  )
