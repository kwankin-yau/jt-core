...
object MBEncoder_1200_UpExgMsg_SiChuan extends VehRelatedMsgBodyEncoder {
  ...
  override def encodeSubBody(msg: JT809VehRelatedMsg, out: ByteBuf): Unit = {
    msg match {
      ...
      case m: JT809Msg_1240_UpExgMsgSafetyTerminal_SiChuan =>
        out.writeFixedLenStr(m.getPlatformId, 11)
        out.writeFixedLenStr(m.getProducer, 50)
        out.writeFixedLenStr(m.getTerminalModel, 30)
        out.writeFixedLenStr(m.getTerminalId, 20)
        out.writeLong(m.getInstallTime)
        out.writeFixedLenStr(m.getInstallCompany, 50)
        out.writeFixedLenStr(m.getContacts, 20)
        out.writeFixedLenStr(m.getTel, 20)
        out.writeByte(if (m.isCompliance) 1 else 0)
    }
  }
