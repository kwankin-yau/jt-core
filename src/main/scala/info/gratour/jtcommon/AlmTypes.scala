...
object AlmTypes {
  ...
  val SiChuanAlmTypes: Seq[String] = JT809Rev2019AlmTypes ++ Seq(
    "0101",
    "0102",
    "0103",
    "0104",
  ...
  def almTypToStr(typ: Int): String = StringUtils.intToHex(typ, 4).toUpperCase

  def strToAlmTyp(typ: String): Int = Integer.parseInt(typ, 16)

  def almTypeName(almTypId: String): String = {
  ...
  def almTypeName(typ: Int): String = {
    val almTypId = StringUtils.intToHex(typ, 4).toUpperCase
    almTypeName(almTypId)
  }

  def almSubTypName(subTyp: String): String = {
    subTyp match {
